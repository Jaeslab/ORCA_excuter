...
class gen_inp:
    ...
    def tddft_text_gen(self):
        ...
        if self.calc in (2, 3, 5, 6, 7, 8):
            ...
            if self.finegird == 1:
                temp += "\n Grid5 FinalGrid6"
            if self.calc in (2, 3, 7) and self.printlevel > 0:
    ...
    def gen_text(self):
        ...
        solvation_text = self.solvation_text_gen()
        esd_text = self.esd_text_gen()
        spec_text = self.spec_text_gen()
        xyz_text = self.xyz_text_gen()
        
        self.total_text = "# " + inst_text + "! " + cond_text + calc_text + tddft_text + solvation_text + esd_text + spec_text + xyz_text

    def text_save(self): #해당 계산 바탕으로 폴더 생성 및 inp파일 저장.
        is_folder = 0
        _folder_name = self.file_text[:-4]
        _xyz_lines = ""
        try:
            os.mkdir(f"{_folder_name}")
            is_folder = 1
        except:
            if os.path.exists(f"{_folder_name}"):
                is_folder = 1
        if is_folder == 1:
            with open(f"./{_folder_name}/"+self.file_text, 'w', encoding = "utf-8") as f:
        ...
        try:
            with open(self.molname+".xyz", 'r') as f:
                _xyz_lines = f.readlines()
            with open(f"./{_folder_name}/"+self.molname+".xyz", 'w') as t:
                t.writelines(_xyz_lines)
                t.close()
        except:
            print("xyz 복붙 실패")
        return 0
...
class chooser:
    ...
    def mol_chooser(self):
        ...
        while status:
            ...
            if chk == "y":
                ...
                # [수정] text_save가 계산별 하위폴더에 inp를 저장하므로 한 단계 하위까지 검색
                    inp_files = glob.glob("*.inp") + glob.glob("*/*.inp")
                    if not inp_files:
                    ...
                    self.geninp.molname = os.path.basename(chosen_inp)[:-4]
    ...
    def tddft_checker(self):
        ...
        if self.geninp.calc in (2,3):
            chk2 = self.ask_yn("NTO 계산을 수행하시겠습니까 [y/n]")
            if chk2 == "y":
                self.geninp.nto = 1
            chk = self.ask_yn("grid6를 적용하시겠습니까?(인광용) [y/n]")
            if chk == "y":
                self.geninp.finegird = 1
            chk3 = self.ask_yn("magnetic transition diopole 보쉴? [y/n]")
            if chk3 == "y":
    ...
    def cpcm_checker(self):
        chk = self.ask_yn("Solvation을 적용하시겠습니까? [y/n]")
        if chk == "y":
            self.geninp.solv = 1
            self.geninp.ep = float(self.ask("용매의 상대유전율을 적으시오 [hint: toluene(2.4)]"))
...
class excuter:
    ...
    def run(self, geninp):
        if geninp.calc != 9:
            inp_name = geninp.file_text[:-4]
            inp_path = "./"+inp_name+"/"+inp_name+ ".inp"
            out_path = "./"+inp_name+"/"+inp_name + ".out"
        else:
            inp_name = geninp.file_text[:-4]
            inp_path = inp_name+ ".inp"
            out_path = inp_name + ".out"
